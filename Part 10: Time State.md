...
set :intensity, 3000
```
Yukarıdaki örnekte, aynı anahtar kelime altındaiki değer tutmaya çalıştığımız için, son **set** fonksiyonundaki değer kazanır. **:intensity** kelimesi ile saklanan değer **3000** olacaktır. 
...
**get** ve **set** ‘i threadler arası kullanmanın kullanmanın iyi taraflarından birisi de oynat’a her bastığınızda aynı sonucu elde edecek olmanızdır. Durmayın, deneyin:
```ruby
{run: 0, time: 0.0}
 └─ 125.72265625
...
### Basit, Deterministik bir State Sistemi
...
set :foo, 1
```
Bu örnekte ilk olarak **:foo** event’inin *Time State*’e eklenmesini bekleyen bir thread oluşturduk. Sonra 2 vuruş bekleyip :foo ‘yu 1 olarak set ‘ledik. Bu komut sync ‘i tamamlamış olur ve thread bir sonraki komuta geçerek :ambi_lunar_land sample’ını çalmaya başlar.
